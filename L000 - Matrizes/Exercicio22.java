/*Juliany Eufrásio
Lista 01 - Exercicio 22
Escreva um programa em Java para gerar uma matriz (N x M) e outra (N
...
public class Exercicio22 {
	...
	public static void main (String[] args) {
		...
		//intercalar as matrizes a e b
		int h = 0;
		for(int i = 0; i < n; i++){
			for(int j = 0; j < m + p; j++){
			if(j < m){
				matrizC[i][h++] = matrizA[i][j];
				}else{
				matrizC[i][h++] = matrizB[i][j];
				}
			}
		}
